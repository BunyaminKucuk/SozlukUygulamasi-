...
namespace Dictionary
{
    class kelimeIslemleri
    {
        ...
        public void kelimeSil(string Turkçesi)
        {
            if(db.baglanti.State==ConnectionState.Open)
            {
                db.baglanti.Close();
            ...
            {
                db.baglanti.Open();
                SqlCommand sil = new SqlCommand("delete kelimeler where trim(TurkceAnlami)=@p1", db.baglanti);
                sil.Parameters.AddWithValue("@p1", Turkçesi.Trim());
                sil.ExecuteNonQuery();
            }                
            catch(Exception hata)
            {
